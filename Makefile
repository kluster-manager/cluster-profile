...
KUBE_NAMESPACE    ?= open-cluster-management-cluster-profile
REGISTRY_SECRET   ?=
...
install:
	...
	helm upgrade -i cluster-profile-manager charts/cluster-profile-manager --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall cluster-profile-manager --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/cluster-profile run \
		--v=3 \
		--secure-port=8443 \
