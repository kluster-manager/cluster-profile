...
func EnableFeatures(ctx context.Context, kc client.Client, profileBinding *profilev1alpha1.ManagedClusterProfileBinding, featureInfo map[string][]string, profile *profilev1alpha1.ManagedClusterSetProfile) error {
	...
	defer func() {
		if shutdownErr := fakeServer.FakeSrv.Shutdown(ctx); shutdownErr != nil {
			logger.Error(shutdownErr, "failed to shut down fake server")
		}
	}()
...
func generateHelmReleaseForFeature(kc client.Client, fs *uiapi.FeatureSet, feature *uiapi.Feature, chart *repo.ChartExtended, curValues map[string]any, mc *v1.ManagedCluster) error {
	...
	if featureDefaultValue == nil {
		featureDefaultValue = make(map[string]any)
	}
